...
(async () => {
  ...
  const translatedTexts = (
    await translateOfflineSugoiCt2LongList(listExtractedFileName, 1)
  ).map((txt) => {
    if (txt === "Base") return "base";
    return txt.replace(/ryouu/i, "Ryou").replace(/Haru/g, "Hal Kris");
  });
  // console.log(listFileName, translatedTexts);
  await Promise.all(
    translatedTexts.map(async (translatedText, index) => {
      return await textToImage.generate(translatedText, {
        maxWidth: width,
        customHeight: height,
        debug: true,
        ...
        textColor: "black",
        textAlign: "center",
        debugFilename: `./raw_images_output/${listFileName[index]}`,
      });
    })
  );
  await delay(100000);
